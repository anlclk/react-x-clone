...
export default function UserPosts({ user, setUser }) {
    ...
    useEffect(() => {
        const userposts = async() => {            
            const { data: tweetler, error } = await supabase.from('tweetler').select('id, created_at, content, profiles(id, username, email)').eq('profile_id', userID);
    ...
    return(
        <div>anıl</div>
        // getUserPosts.map(x => ( 
        // <div className="tweetArea" key={x.id}>
